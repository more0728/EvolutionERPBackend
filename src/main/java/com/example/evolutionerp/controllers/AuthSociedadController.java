...
import com.example.evolutionerp.dtos.AuthSociedadRequest;
...
public class AuthSociedadController {
  ...
  @PostMapping("/sociedad") public ResponseEntity<JwtResponseSociedadDTO> seleccionar(@RequestBody AuthSociedadRequest req, Authentication auth){
    String codSoc=req.getCodSociedad();
    if(!socRepo.existsById(codSoc)) return ResponseEntity.badRequest().build();
  ...
  @GetMapping("/sociedades") public ResponseEntity<List<String>> sociedades(){ return ResponseEntity.ok(socRepo.findAll().stream().map(s->s.getCodSociedad()).collect(Collectors.toList())); }
}
